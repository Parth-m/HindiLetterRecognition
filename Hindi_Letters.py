...
def main():
    ...
    while (cap.isOpened()):
        ...
        cnts,heir = cv2.findContours(thresh.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        center = None
        if len(cnts) >= 1:
            ...
            if len(pts) != []:
                ...
                blackboard_cnts,heir2= cv2.findContours(thresh1.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
